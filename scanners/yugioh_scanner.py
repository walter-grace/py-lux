#!/usr/bin/env python3
"""
AI-Powered eBay Scanner for PSA Arbitrage
Uses deep research agent to:
1. Search eBay for Yu-Gi-Oh! PSA 10 1st Edition cards
...
def main():
    """Main entry point for AI eBay scanner."""
    # Load environment
    load_dotenv(".env")
    ...
    # Get parameters
    limit = 20
    year = "2002"  # Default to 2002 for Yu-Gi-Oh!
    tax_rate = 0.09
    min_spread = 0
    min_spread_pct = 0
    ...
    print("=" * 70)
    print("AI-Powered eBay Scanner for PSA Arbitrage")
    print("=" * 70)
    print(f"Searching eBay for PSA 10 Yu-Gi-Oh! 1st Edition cards")
    if year:
    ...
    ebay_items = search_trading_cards(limit, env, year=year, game="yugioh")
    ...
    if rapidapi_key and ebay_items:  # Only search if we found eBay items
        ...
        try:
            ...
            fallback_query = f"PSA 10 yugioh 1st edition {year}" if year else "PSA 10 yugioh 1st edition"
    ...
    csv_filename = "data/yugioh_cards.csv"
    ...
    with open("data/yugioh_arbitrage_opportunities.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "cert_number", "title", "card_name", "year", "set",
            "ebay_price", "shipping", "est_tax", "all_in_cost",
    ...
    print(f"\nSaved {len(arbitrage_deals)} arbitrage opportunities to arbitrage_opportunities.csv")
